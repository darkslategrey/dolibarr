    ...
    if ($num)
    {
        ...
        while ($i < $num)
        {
            ...
            if (! empty($conf->agenda->enabled) && $user->rights->agenda->myactions->create)
            {
                ...
                /* print img_object($langs->trans("Rendez-Vous"),"action_rdv") . '</a>'; */
                print '<td><a href="'.DOL_URL_ROOT.'/comm/action/fiche.php?action=create&actioncode=&contactid='.$obj->rowid.'&socid='.$object->id.'&backtopage='.urlencode($backtopage).'">';

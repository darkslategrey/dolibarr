...
function pdf_bank(&$pdf,$outputlangs,$curx,$cury,$account,$onlynumber=0)
{
    ...
    {
        $pdf->SetFont('','B',8);
        $pdf->MultiCell(80, 3, $outputlangs->transnoentities('PaymentByTransferOnThisBankAccount').':', 0, 'L', 0);
        $cury+=4;
    }
    ...
    $pdf->SetXY ($curx, $cury);
    $pdf->MultiCell(80, 3, $outputlangs->transnoentities($ibankey).': ' . $outputlangs->convToOutputCharset($account->iban), 0, 'L', 0);
    $pdf->SetXY ($curx, $cury+3);
    $pdf->MultiCell(80, 3, $outputlangs->transnoentities($bickey).': ' . $outputlangs->convToOutputCharset($account->bic), 0, 'L', 0);
...
function pdf_pagefoot(&$pdf,$outputlangs,$paramfreetext,$fromcompany,$marge_basse,$marge_gauche,$page_hauteur,$object,$showdetails=0)
{
    ...
    if ($line)	// Free text
    {
      $pdf->SetXY($marge_gauche,-($posy+45));
